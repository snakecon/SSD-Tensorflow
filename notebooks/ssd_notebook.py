...
net_shape = (300, 300)
data_format = 'NHWC'
img_input = tf.placeholder(tf.uint8, shape=(None, None, 3), name="image_raw")
...
train_writer = tf.summary.FileWriter("board", isess.graph)
...
node_names = "ssd_300_vgg/block4_box/Reshape," \
...
checkpoint_path = saver.save(isess, os.path.join("export", 'model.ckpt'))
tf.train.write_graph(isess.graph, "export", input_graph_name)

from tensorflow.python.tools import freeze_graph
export_graph = tf.Graph()
with export_graph.as_default():
    freeze_graph.freeze_graph(input_graph=os.path.join("export",
                                                       input_graph_name),
                              input_saver="",
                              input_binary=False,
                              ...
                              output_graph=os.path.join("export",
                                                        output_graph_name),
                              clear_devices=True,
